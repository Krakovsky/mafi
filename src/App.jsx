...
import { Clone, Environment, OrbitControls, Stars, useGLTF, useTexture, Html } from '@react-three/drei'
...
function Player({ player, position, focused }) {
  ...
  // Смещение камеры по высоте, чтобы не перекрывались
  const cameraYOffset = 3.1 + player.id * 0.12 // чуть-чуть разнести по высоте
  // Смещение по радиусу, чтобы iframe не пересекались (немного наружу)
  ...
  // Чем дальше от камеры зрителя (и чем больше боковое смещение), тем крупнее вебка.
  ...
  return (
    <>
      ...
      {/* Вебкамера над головой игрока, всегда смотрит прямо на зрителя */}
      <Html
